	...
	while (!feof(fp))//feof(fp)检查文件是否结束，如果结束则返回非零值，否则返回0
	{
		...
		*/
		...
		if (!img.empty()){
			...
			sprintf(cSavePath, "results\\%s.jpg", strImgName.c_str());	//v2.3.2
			imwrite(cSavePath, rec_img);
			//imwrite(cSavePath, G_Y);
			//imwrite(cSavePath, chan[1]);
			//imwrite(cSavePath, binaryImg_txt);
			//imwrite(cSavePath, binaryImg_txt0);
			//imwrite(cSavePath, binaryImg_background);
			//imwrite(cSavePath, bg);
		}
		else{
			continue;
		}
